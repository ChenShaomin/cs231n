...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dxt, dhtp, dWxt, dWht, dbt = rnn_step_backward(dh[:, t, :] + dhtp, cache[t])
        dx[:, t, :] = dxt
        dWx = dWx + dWxt
        dWh = dWh + dWht
        db = db + dbt
...
def word_embedding_forward(x, W):
    ...
    for n in range(N):
        for t in range(T):
            x_onehot[n, t, x[n, t]] = 1
    out = np.dot(x_onehot, W)
    """
    out = W[x, :]
    cache = [x, W]
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    da = np.hstack((i*(1-i)*di, f*(1-f)*df, o*(1-o)*do, (1-g**2)*dg))  #(N, 4H)
...
def lstm_backward(dh, cache):
    ...
    dc = np.zeros((N, H))
    for t in range(T-1, -1, -1):
        dx[:, t, :], dprev_h, dc, dWx_step, dWh_step, db_step = lstm_step_backward(dh[:, t, :] + dprev_h, dc, cache[t])
        dWx, dWh, db = dWx + dWx_step, dWh + dWh_step, db + db_step
    dh0 = dprev_h
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
